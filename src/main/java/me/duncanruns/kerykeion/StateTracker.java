...
class StateTracker {
    private static final Map<HermesInstance, UpdatingJsonFileReader> stateFiles = new HashMap<>();

    public TickResult tick(Collection<HermesInstance> instances) {
        TickResult tickResult = new TickResult();
        stateFiles.keySet().removeIf(i -> !instances.contains(i));
        for (HermesInstance instance : instances) {
            UpdatingJsonFileReader reader = stateFiles.computeIfAbsent(instance, i -> new UpdatingJsonFileReader(i.getGameDir().resolve("hermes").resolve("state.json")));
            try {
                reader.read().ifPresent(
                        json -> tickResult.entries.add(new TickResult.EntryWithInstance(json, instance))
                );
            } catch (IOException | JsonSyntaxException e) {
                Kerykeion.errorLogger.accept("Failed to read state file", e);
            }
        }
        return tickResult;
    }
